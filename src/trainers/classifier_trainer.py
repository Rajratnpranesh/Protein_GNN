...
class ClassifierTrainer(BaseTrainer):
    ...
    def build_model(self, model_type='gcnn', optimizer='Adam', learning_rate=0.001, nb_classes=2, **model_args):
        ...
        self.operators = [opt, loss, out, acc_update, prec_update, recall_update, auc_update, F_flatten]
        if nb_classes > 2:
    ...
    def train_epoch(self, data_loader, sess):
        ...
        for i, data in enumerate(bg(data_loader)):
            ...
            F_flatten = out[-1]
            print(F_flatten)
            save_vec = open(str(j)+".pickle" ,"wb")
            pickle.dump(F_flatten, save_vec)
            save_vec.close()
            ...
            #xyz = tf.print(F_flatten)

            #f= open("vector_.txt " ,"w+")
            #f.write(str(xyz))
            #f.close()
